...
@section('content')
    <div class="w-screen min-h-screen pt-[7vh] flex flex-col items-center justify-center p-8">
        <h1 class="text-4xl font-bold mb-8">Requests Management</h1>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse ($allRequests as $request)
                                <tr class="hover:bg-blue-50 transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="text-sm font-semibold text-gray-800">{{ $request->package->name }}</span>
                                        <br>
                                        <span class="text-xs text-gray-500">Rp {{ number_format($request->package->price, 0, ',', '.') }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-sm font-medium {{ $status === 'pending' ? 'bg-yellow-100 text-yellow-800' : ($status === 'ongoing' ? 'bg-blue-100 text-blue-800' : ($status === 'rejected' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800')) }}">
                                            {{ ucfirst($status) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            <x-pages.action-link href="{{ route('admin.request.edit', $request->id) }}" color="yellow" textColor="white">
                                                Edit
                                            </x-pages.action-link>
                                        </div>
                                    </td>
